...
59種のツール、長期記憶、cronベースの自律スケジューラ、Web機能、コード生成+GitHub CI、汎用HTTP、[SKILL.md標準](https://agentskills.io)準拠の拡張可能スキルシステムを備えた自律AIエージェント型Discord Bot。[OpenRouter](https://openrouter.ai) 経由で任意のLLMを利用可能（デフォルト: Gemini 2.5 Flash）。

Python 16ファイル・約5,400行。[discord.py](https://github.com/Rapptz/discord.py) 使用。

## 機能

- **LLMエージェント** — OpenRouter経由で任意のモデルを利用、`/model` で実行時に切り替え可能
- **59種のツール** — Discord管理・記憶・スケジューラ・Web・システム・コード生成・HTTP・スキルの8カテゴリ
...
skills/*/SKILL.md       スキルパッケージ（画像生成・翻訳・動画・データ分析）
